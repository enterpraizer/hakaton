...
vm_network_association = Table(
    ...
    Base.metadata,
    Column("vm_id", Uuid(as_uuid=True), ForeignKey("virtual_machines.id", ondelete="CASCADE"), primary_key=True),
    Column("network_id", Uuid(as_uuid=True), ForeignKey("virtual_networks.id", ondelete="CASCADE"), primary_key=True),
)
